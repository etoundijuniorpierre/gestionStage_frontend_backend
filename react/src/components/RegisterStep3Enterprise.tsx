...
type Props = {
  onPrev: () => void;
  onFinish: (data: EnterpriseFormData) => void;
};

const RegisterStep3Enterprise = ({ onPrev, onFinish }: Props) => {
  const { formData, setStep } = useRegistrationStore();
  const { register, handleSubmit, setValue, formState: { errors, isValid } } = useForm<EnterpriseFormData>({
    mode: 'onChange',
    defaultValues: {
      name: formData.name || '',
...
export default RegisterStep3Enterprise; 