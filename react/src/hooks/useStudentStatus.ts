...
interface StudentStatus {
  ...
  pendingApplications: ApplicationResponseDto[];
  approvedApplications: ApplicationResponseDto[];
  acceptedApplications: ApplicationResponseDto[];
  loading: boolean;
  refresh: () => Promise<void>;
}

export const useStudentStatus = (): StudentStatus => {
  const [pendingApplications, setPendingApplications] = useState<ApplicationResponseDto[]>([]);
  const [approvedApplications, setApprovedApplications] = useState<ApplicationResponseDto[]>([]);
  const [acceptedApplications, setAcceptedApplications] = useState<ApplicationResponseDto[]>([]);
  ...
  const fetchApplications = async () => {
    try {
      ...
      const pendingData = (pendingRes?.data as ApplicationResponseDto[]) || [];
      const approvedData = (approvedRes?.data as ApplicationResponseDto[]) || [];
  ...
  const hasApplicationForOffer = (offerId: number): boolean => {
    return [...pendingApplications, ...approvedApplications, ...acceptedApplications]
      .some((app: ApplicationResponseDto) => app.offer?.id === offerId);
  };
